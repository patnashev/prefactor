...
namespace arithmetic
{
    ...
    public:
        ...
        bool operator == (const PolyCoeff& a) { return _flag == a._flag && (is_small() || *_value == *a._value); }
    ...
    {
    public:
        Poly(GWArithmetic& gw, int size) : _gw(gw)
        {
            for (int i = 0; i < size; i++)
                emplace_back();
        }
        Poly(Poly&& a) : vector(std::move(a)), _gw(a.gw()) { }
        virtual ~Poly() { }
        Poly& operator = (Poly&& a) { std::vector<PolyCoeff>::operator=(std::move(a)); return *this; }
    ...
    public:
        ...
        void karatsuba(const Poly& a, const Poly& b, PolyCoeff* res, int size, int level = 0);
        void karatsuba_half(const Poly& a, const Poly& b, PolyCoeff* res, int size, int half, int level = 0);
        void karatsuba_halfother(const Poly& a, const Poly& b, PolyCoeff* res, int size, int half, int level = 0);
